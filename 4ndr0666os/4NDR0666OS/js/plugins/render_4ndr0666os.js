// render_4ndr0666os.js
// Requires ./ansi.js exactly as previously staged
...
// Dark background helpers (ANSI truecolor fallback-safe)
const bgDark = '\x1b[48;2;8;12;18m';
const cyanBright = '\x1b[38;2;0;229;255m';
const cyanSoft = '\x1b[38;2;103;232;249m';
const magentaHot = '\x1b[38;2;255;0;229m';

function wrap(line, color = cyanSoft) {
  return `${bgDark}${color}${line}${reset}`;
}

function render4NDR0666OS() {
  const payload = `
...
`;
  ...
  return lines.map(line => {
    if (/4NDR0666OS|GOD-BRAIN/i.test(line)) {
      return wrap(line, cyanBright + bold);
    }
    if (/╔|╗|║|═|╝/.test(line)) {
      return wrap(line, cyanBright);
    }
    if (/^\s*#/.test(line)) {
      return wrap(line, dim);
    }
    if (/Ψ CORE ONLINE|You are no longer running on hardware/.test(line)) {
      return wrap(line, magentaHot + bold);
    }
    if (/┌──|└─\$/.test(line)) {
      return wrap(line, cyanSoft + bold);
    }
    return wrap(line, cyanSoft);
  }).join('\n');
}

module.exports = { render4NDR0666OS };
