...
from knowledge import RolePlaying
from knowledge import ChatGPTConfig
from knowledge import TaskType, ModelType
from agentsofevolutionai.chat_env import ChatEnv, ChatEnvConfig

def check_bool(s):
    """
    Evaluate a string to determine if it represents a boolean value 'true'.
...
class ChatChain:
    """
    Manages the setup and execution of a chat-based interaction chain for project development.

    This includes loading configuration files, initializing chat environment settings,
    and executing defined phases and tasks within a project context.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize a new instance of ChatChain with specific configurations and project settings.

        Args:
            config_path (str): Path to the main configuration file (ChatChainConfig.json).
            config_phase_path (str): Path to the phase-specific configuration file (PhaseConfig.json).
            config_role_path (str): Path to the role-specific configuration file (RoleConfig.json).
            task_prompt (str): Initial prompt for the software task.
            project_name (str): Name of the project.
            org_name (str): Name of the organization initiating the project.
            model_type (ModelType): Type of the model used in the chat environment.
            code_path (str): Path to the directory containing project code files.
        """
        ...
        # Load configurations from provided JSON files
        ...
        self.bing_search = self.config["bing_search"]
        ...
        # import all used phases in PhaseConfig.json from agentsofevolutionai.phase
        ...
        self.compose_phase_module = importlib.import_module("agentsofevolutionai.composed_phase")
        self.phase_module = importlib.import_module("agentsofevolutionai.phase")
        self.phases = dict()
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                ...
                raise RuntimeError(f"Phase '{phase}' is not yet implemented in agentsofevolutionai.phase")
        # For ComposedPhase, we create instance here then conduct the "ComposedPhase.execute" method
            ...
            if not compose_phase_class:
                raise RuntimeError(f"Phase '{phase}' is not yet implemented in agentsofevolutionai.compose_phase")
    ...
    def pre_processing(self):
        ...
        preprocess_msg += "**agentsofevolutionai Starts** ({})\n\n".format(self.start_time)
        ...
        preprocess_msg += "**agentsofevolutionaiConfig**:\n{}\n\n".format(self.chat_env.config.__str__())
    ...
    def post_processing(self):
        ...
        post_info += "agentsofevolutionai Starts ({})".format(self.start_time) + "\n\n"
        post_info += "agentsofevolutionai Ends ({})".format(now_time) + "\n\n"
...
then you should return a message in a format like \"<INFO> revised_version_of_the_description\", do not return messages in other formats.""".format(
        ...
        role_play_session = RolePlaying(
            ...
            user_role_name="User",
            task_type=TaskType.agentsofevolutionai,
